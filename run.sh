...
if hash docker 2>/dev/null; then
    # for docker lovers
    docker build . -t oopsaleem/kafka-connect-ericsson-node-alarms:1.0
    docker run --net=host --rm -t \
           -v $(pwd)/offsets:/kafka-connect-ericsson-node-alarms/offsets \
    ...
    $CONFLUENT_HOME/bin/connect-standalone $CONFLUENT_HOME/etc/schema-registry/connect-avro-standalone.properties config/DemoSourceConnectorExample.properties
else
